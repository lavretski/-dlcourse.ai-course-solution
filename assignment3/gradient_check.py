...
def check_gradient(f, x, delta=1e-5, tol=1e-4):
    ...
    orig_x = x.copy()
    #if len(x.shape) == 1:
    #    orig_x = orig_x[np.newaxis, :]

    fx, analytic_grad = f(orig_x)
    assert np.all(np.isclose(orig_x, x, tol)), "Functions shouldn't modify input variables"

    assert analytic_grad.shape == orig_x.shape
    analytic_grad = analytic_grad.copy()

    it = np.nditer(orig_x, flags=['multi_index'], op_flags=['readwrite'])
    while not it.finished:
        ix = it.multi_index
        analytic_grad_at_ix = analytic_grad[ix]
        delta_x = np.zeros_like(orig_x)
        delta_x[ix] = delta
        f_plus_delta_x, _ = f(orig_x + delta_x)
        f_minus_delta_x, _ = f(orig_x - delta_x)
        numeric_grad_at_ix = (f_plus_delta_x - f_minus_delta_x) / (2 * delta)
