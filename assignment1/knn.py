...
class KNN:
    ...
    def predict_labels_multiclass(self, dists):
        ...
        indexes = np.arange(dists.shape[1])
        for i in range(num_test):
            indexes = sorted(indexes, key = lambda j: dists[i][j])
            pred[i] = self.find_most_repeated_label(self.train_y[indexes][:self.k])
        return pred

    def find_most_repeated_label(self, labels):
        d = {}
        for label in labels:
            if label in d:
                d[label] += 1
